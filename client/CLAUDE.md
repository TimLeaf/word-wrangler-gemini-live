...
- **ローカル開発（Firestore Emulator）**: `client/firebase.json` に最小設定（firestore port 8080, UI 無効）を置いてある
  ```bash
  # ターミナル1: Emulator
  cd client && npx firebase-tools emulators:start --only firestore --project demo-wordbook
  # ターミナル2: client を Emulator 接続で起動
  cd client && FIRESTORE_EMULATOR_HOST=localhost:8080 npm run dev
  ```
  Emulator はメモリ上のため再起動でデータは消える。`firestore-debug.log` / `.firebase/` は `.gitignore` 済み
