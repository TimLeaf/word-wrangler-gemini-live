...
describe("useGameState (INV-3)", () => {
  ...
  it("initializeGame() で ACTIVE に遷移し、各カウンタが初期化される (INV-3 の核)", () => {
    const { result } = renderHook(() => useGameState());
  ...
  it("initializeGame() で localStorage が空のとき bestScore は 0", () => {
    const { result } = renderHook(() => useGameState());
  ...
  it("initializeGame() で localStorage の bestScore を読み込む", () => {
  ...
  it("finishGame() で FINISHED に遷移する", () => {
    const { result } = renderHook(() => useGameState());
  ...
  it("moveToNextWord() で currentWord が次の語に進む", () => {
    const { result } = renderHook(() => useGameState());
    ...
    expect(result.current.currentWord).toBe(result.current.words[1]);
  });

  it("moveToNextWord() で末尾に達すると単語プールを再充填して index が 0 に戻る", () => {
    const { result } = renderHook(() => useGameState());
    ...
    expect(result.current.currentWord).toBe(
      initialWords[GAME_CONFIG.WORD_POOL_SIZE - 1]
    );

    // もう 1 回呼ぶと再充填され、index が 0 に戻る
    act(() => {
      result.current.moveToNextWord();
    });

    expect(result.current.words.length).toBe(GAME_CONFIG.WORD_POOL_SIZE);
    expect(result.current.currentWord).toBe(result.current.words[0]);
  });
