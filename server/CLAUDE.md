...
uv run pc cloud secrets set word-wrangler-secrets --file .env  # secret_set を登録 / 更新（.env から）
uv run pc cloud deploy                    # Pipecat Cloud にデプロイ（PCC 側でリモートビルドが走るため build-push 不要）
```
...
```
transport.input()
  → RTVIProcessor         # クライアント / サーバ間の双方向イベント中継（PipelineWorker が自動追加）
...
`bot.py:197-210` の `Pipeline([...])` には 5 段だけが書かれているが、`PipelineWorker` の生成時にランタイムが `RTVIProcessor` を自動追加する。`bot.py` では `@worker.rtvi.event_handler("on_client_ready")`（`bot.py:220`）でこの自動追加されたプロセッサにハンドラを登録し、クライアント接続時に `LLMRunFrame` をキューイングして会話を開始する。Pipecat 1.3.0 でランタイム型が単一パイプライン向けの `PipelineTask` / `PipelineRunner` からマルチ Worker 用の `PipelineWorker` / `WorkerRunner` に一括リネームされたため、本リポジトリも同 API を使う。起動は `runner = WorkerRunner(...)` → `await runner.add_workers(worker)` → `await runner.run()` の三段になる（`bot.py:235-238`）。

重要なポイント：

- **STT は使わない**：`GeminiLiveLLMService` が音声入力を直接処理するため、STT サービスはパイプラインに無い
- **ミュート戦略 `MuteUntilFirstBotCompleteUserMuteStrategy`**：`LLMUserAggregatorParams.user_mute_strategies` に渡す形で `user_aggregator` 内部に統合されている（`bot.py:192`）。ボットの自己紹介が完了する前にユーザーが割り込むのを防ぎ、クライアント側のゲーム開始トリガー（`BotStoppedSpeaking` の初回発火）と対になっている
...
- **Krisp ノイズフィルタ**：本番環境（`ENV != "local"`）では `KrispVivaFilter`（Krisp VIVA SDK ベース）が `DailyParams.audio_in_filter` として適用される（`bot.py:243-256`）。利用には `krisp_audio` パッケージが必要
- **VAD**：`SileroVADAnalyzer()` をデフォルト引数で `LLMUserAggregatorParams.vad_analyzer` に明示指定（`bot.py:193`）
...
Pipecat Cloud に GitHub Actions で自動デプロイ。`main` ブランチに `server/**` または `.github/workflows/deploy-server.yml` の変更がマージされると `.github/workflows/deploy-server.yml` が起動し、`uv sync --locked` → `uv run pc cloud deploy --yes` を実行する。
...
- **Secret (`word-wrangler-secrets`) の更新は手動運用**: GHA からは触らない。`server/.env` を更新したら手元で `uv run pc cloud secrets set word-wrangler-secrets --file .env` を実行する
- 手動再 deploy は `workflow_dispatch`、または手元で `uv run pc cloud deploy --yes` を実行
